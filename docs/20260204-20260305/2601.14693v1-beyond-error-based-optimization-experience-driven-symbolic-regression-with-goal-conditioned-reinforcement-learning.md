...
tags: ["query:sr"]
score: 9.0
evidence: 用于数学表达式发现的目标条件强化学习
tldr: 针对符号回归中仅依赖误差优化导致搜索方向模糊的问题，本文提出EGRL-SR框架。该方法将符号回归建模为目标条件强化学习问题，利用事后经验回放和动作价值网络从历史轨迹中学习结构模式。通过全点满足二元奖励函数和结构引导启发式探索，EGRL-SR在多个基准测试中展现出优于现有方法的恢复率和鲁棒性，能更高效地发现复杂数学表达式。
selection_source: fresh_fetch
motivation: 传统符号回归方法过度依赖拟合误差，导致在具有相似误差但结构迥异的表达式空间中搜索效率低下且难以收敛。
method: 提出基于目标条件强化学习的EGRL-SR框架，结合事后经验回放、二元奖励函数及结构引导探索策略来优化动作价值网络。
result: 在公开基准测试中，EGRL-SR在恢复率、鲁棒性以及复杂表达式发现能力上均显著优于当前最先进的方法。
conclusion: 实验证明通过经验驱动的动作价值网络能有效引导搜索，且所设计的奖励函数和探索策略对提升性能至关重要。
---

## 摘要
符号回归旨在自动识别紧凑且可解释的数学表达式，以建模输入和输出变量之间的函数关系。大多数现有的基于搜索的符号回归方法通常依赖拟合误差来指导搜索过程。然而，在巨大的表达式空间中，许多候选表达式可能表现出相似的误差值，但在结构上却存在实质性差异，这导致了模糊的搜索方向，并阻碍了向底层真实函数的收敛。为了应对这一挑战，我们提出了一个名为 EGRL-SR（经验驱动的目标条件强化学习符号回归）的新型框架。与传统的误差驱动方法不同，EGRL-SR 引入了一个新的视角：利用精确的历史轨迹并优化动作价值网络，以主动引导搜索过程，从而实现更稳健的表达式搜索。具体而言，我们将符号回归建模为一个目标条件强化学习问题，并结合了事后经验回放（hindsight experience replay），使动作价值网络能够从多样的输入输出对中泛化出通用的映射模式。此外，我们设计了一个全点满足二元奖励函数，鼓励动作价值网络关注结构模式而非低误差表达式，并同时提出了一种结构引导的启发式探索策略，以增强搜索的多样性和空间覆盖率。在公共基准测试上的实验表明，EGRL-SR 在恢复率和稳健性方面始终优于现有最先进的方法，并且在相同的搜索预算下可以恢复更复杂的表达式。消融实验结果验证了动作价值网络能有效地引导搜索，且奖励函数和探索策略都发挥了关键作用。
